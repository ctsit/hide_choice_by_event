$(document).ready(function() {
    ...
    $('body').on('dialogopen', function(event, ui) {
        ...
        var descr = 'Hides options for categorical fields based on what event they are on. As input, this tag requires a JSON object that describes when to hide a categorical field. For example: @HIDE-CHOICE-BY-EVENT=[{"code": "1", "event":["demography", "check-up"]}, {"code": "2", "event":["demography"]}] would hide option "1" on the "demography" and "check-up" events. Similarly it would hide option "2" on only the "demography" event. Note that the "code" key corresponds to one of the optional values the categorical field can have, while "event" corresponds to a list of valid events for this project. Currently this action tag only supports radio buttons. For more guidence on how to format this JSON check out our <a href="https://github.com/rajputd/hide_choice_by_event" style="font-size:inherit;"><strong>documentation</strong></a>.';
